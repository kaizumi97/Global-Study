...
embedded_check_all <- function(origtext, newtext, i) {
    row_label <- i - 13  # Row number shown in error messages
  error_messages <- c()
  
  embedded_text <- read_embedded(origtext, newtext, row_label)
  if (embedded_text != "correct") {
    error_messages <- c(error_messages, embedded_text)
  }
  
  bold_text <- read_bold(origtext, newtext, row_label)
  if (bold_text != "correct") {
    error_messages <- c(error_messages, bold_text)
  }
  
  ul_text <- read_ul(origtext, newtext, row_label)
  if (ul_text != "correct") {
